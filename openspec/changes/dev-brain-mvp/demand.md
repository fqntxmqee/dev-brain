...
| L5 | L5-BRAIN-01 | 飞书发需求 → 收到任务计划卡片 | 草拟 |
| L5 | L5-BRAIN-02 | 批准后 Lead 拆分 DAG 并展示子任务 | 草拟 |
| L5 | L5-BRAIN-03 | Claude/Codex/Cursor 各完成一次子任务 | 草拟 |
| L5 | L5-BRAIN-04 | 跨 Agent 文件锁阻止冲突写入 | 草拟 |
| L5 | L5-BRAIN-05 | 飞书收到最终汇总报告 | 草拟 |

### 3.2 范围

**In Scope (Phase 1)**:
- dev-brain 项目骨架与 OpenSpec 产物
- Feishu Gateway（lark-cli event 订阅 + 消息路由）
- Brain Engine（任务计划、审批门控、状态查询）
- Agent Adapter 接口 + cc-connect / Cursor 适配器骨架
- 内置 TaskOrchestrator + 审批门控

**Out of Scope (Phase 1)**:
- 真实 cc-connect API 协议完整实现（Phase 2）
- 真实 @cursor/sdk 集成（Phase 2）
- cc-connect 三 Bot 下线改造
- 长期记忆检索
