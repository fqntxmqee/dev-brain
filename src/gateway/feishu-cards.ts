...
function statusEmoji(
  status: ExecutionProgress["subTasks"][number]["status"],
): string {
  switch (status) {
    case "pending":
      return "⏳";
...
export function buildPlanCard(
  ...
  const subTaskLines = plan.subTasks
    .map((st, i) => {
      ...
      return `${i + 1}. **${st.runtime}** ${st.description.slice(0, MAX_CARD_FIELD_LEN)}${deps}`;
    })
    .join("\n");
  ...
  const elements: Record<string, unknown>[] = [
    {
      ...
      text: {
        tag: "lark_md",
        content: `**需求**\n${plan.description.slice(0, MAX_DESC_LEN)}`,
      },
    },
    { tag: "hr" },
    {
    ...
    elements: [
      {
        ...
        text: {
          tag: "lark_md",
          content: `**#${progress.taskId.slice(0, SHORT_ID_LEN)}** ${progress.description.slice(0, MAX_SUBTASK_TITLE_LEN)}`,
        },
      },
      { tag: "hr" },
      {
  ...
  return {
    ...
    elements: [
      ...
      { tag: "hr" },
      {
        ...
        text: {
          tag: "lark_md",
          content: lines || result.summary.slice(0, MAX_DESC_LEN),
        },
      },
    ],
  };
