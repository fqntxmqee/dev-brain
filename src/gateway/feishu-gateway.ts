...
import type { BrainTaskPlan, FeishuCardAction, FeishuInboundMessage } from '../core/types.js';
import { buildPlanCard, buildProgressCard, buildSummaryCard } from './feishu-cards.js';
import {
...
export class FeishuGateway {
  ...
  async handleCardAction(action: FeishuCardAction): Promise<void> {
    ...
    log(`[feishu:card] ${action.operatorName}@${action.chatId}: ${action.action}`);
    ...
    if (this.useCards) {
      await this.sendCard(action.chatId, buildSummaryCard(result, pending.description));
    }
  ...
  async handleMessage(message: FeishuInboundMessage): Promise<void> {
    ...
    switch (intent.type) {
      ...
      case 'approve': {
        ...
        const result = await this.deps.brain.approveAndExecute(message.chatId, async (progress) => {
          if (!this.useCards) return;
          await this.sendCard(message.chatId, buildProgressCard(progress), message.messageId);
        });
  ...
  private async sendText(message: FeishuInboundMessage, text: string): Promise<void> {
...
export async function runFeishuEventLoop(
  ...
  const child = spawn('lark-cli', ['event', '+subscribe', '--format', 'compact'], {
