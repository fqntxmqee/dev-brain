...
export class CursorAdapter implements AgentAdapter {
  ...
  async *send(request: AdapterRequest): AsyncIterable<AdapterEvent> {
    ...
    if (this.config.adapterMode === 'stub' || !this.config.cursorApiKey) {
      ...
      yield { type: 'progress', content: 'cursor adapter (stub)', timestamp: now };
      yield {
        type: 'done',
    ...
    yield { type: 'progress', content: 'cursor: @cursor/sdk Agent.prompt', timestamp: now };
    ...
    try {
      ...
      if (message.includes("Cannot find package '@cursor/sdk'")) {
        yield {
          type: 'progress',
          content: 'cursor: @cursor/sdk not installed, falling back to cc-connect',
          timestamp: new Date().toISOString(),
        };
        yield* this.ccConnectFallback.send(request);
        return;
      ...
      yield {
        type: 'error',
        content: `cursor sdk failed: ${message}`,
        timestamp: new Date().toISOString(),
      };
    }
  }
