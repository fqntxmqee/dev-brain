...
import { InMemoryFeishuReporter, LarkCliFeishuReporter } from '../gateway/feishu-reporter.js';
...
program.name('dev-brain').description('飞书指挥的多 Agent 开发大脑').version('0.4.0');
...
program
  ...
  .description('检查或生成 cc-connect headless 配置（去掉 platforms，仅 Worker）')
  .option('--check', '仅检查当前配置')
  .option('--apply', '备份并原地应用 headless 配置（生产切换）')
  .option('--dry-run', '不写入文件')
  .option('-o, --output <path>', '输出路径', '')
  .action(async (opts: { check?: boolean; apply?: boolean; dryRun?: boolean; output: string }) => {
    const config = loadConfig();
    ...
    const outputPath =
      opts.output.trim() ||
      join(dirname(sourcePath), 'config.headless.toml');
